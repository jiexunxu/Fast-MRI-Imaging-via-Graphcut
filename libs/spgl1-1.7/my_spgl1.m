function [x,r,g,info] = my_spgl1( opts, b, tau, sigma, x, options )
...
x         = project(x,tau);
r         = b - Aprod(x,opts,1);  % r = b - Ax
g         =   - Aprod(r,opts,2);  % g = -A'r
f         = r'*r / 2; 
...
while 1
    ...
    try
       ...
       [f,x,r,nLine,stepG,lnErr] = ...
           spgLineCurvy(x,opts,gStep*g,max(lastFv),@Aprod,b,@project,tau);
       ...
       if lnErr
          ...
          gtd  = g'*dx;
          [f,x,r,nLine,lnErr] = spgLine(f,x,dx,gtd,max(lastFv),@Aprod,opts,b);
          nLineTot = nLineTot + nLine;
       end
       if lnErr
       ...
       if doSubspaceMin
          ...
          ebar   = sign(x(nnzIdx));
          nebar  = length(ebar);
          Sprod  = @(y,mode)LSQRprod(@Aprod,opts,nnzIdx,ebar,n,y,mode);
...
function z = LSQRprod(Aprod,opts,nnzIdx,ebar,n,dx,mode)
...
function [fNew,xNew,rNew,iter,err] = spgLine(f,x,d,gtd,fMax,Aprod,opts,b)
...
function [fNew,xNew,rNew,iter,step,err] = ...
    spgLineCurvy(x,opts,g,fMax,Aprod,b,project,tau)
...
function y=HxTV(x, opts, mode)
    W=opts.W;G=opts.G;sensit=opts.sensit;b=opts.b;fullNbrMat=opts.fullNbrMat;
    conjSensit=opts.conjSensit;fullNbrMatTransp=opts.fullNbrMatTransp;
    decompLvl=opts.decompLvl;wname=opts.wname;M=opts.M;data=opts.data;
    ylength=opts.ylength;
    if mode==1
