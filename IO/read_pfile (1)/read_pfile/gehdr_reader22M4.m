...
function [t, outpos] = gehdr_reader22M4(chr, inpos, type, count)
...
function [t, outpos] = read_APPLICATION_CONFIG_HDR(chr, inpos, count)
  outpos = inpos + 228 * count;
  chr2=reshape(chr(inpos:inpos+count*228-1), 228, []);
  t.error = read_int(chr2(1:4,:), 1, count);
  t.clinicalName = read_char(chr2(5:36,:), 1, count*32);
  t.appConfig = read_APPLICATION_CONFIG_PARAM_TYPE(chr2(37:228,:), 1, count);
...
function [t, outpos] = read_TX_COIL_INFO(chr, inpos, count)
  ...
  t.meanJstd = read_f32(chr2(165:212,:), 1, count*12);
  t.separationStdev = read_f32(chr2(213:216,:), 1, count);

function [t, outpos] = read_PSD_COIL_INFO(chr, inpos, count)
  outpos = inpos + 4084 * count;
  chr2=reshape(chr(inpos:inpos+count*4084-1), 4084, []);
  t.imgRcvCoilInfo = read_COIL_INFO(chr2(1:960,:), 1, count*10);
  t.volRcvCoilInfo = read_COIL_INFO(chr2(961:1920,:), 1, count*10);
  t.txCoilInfo = read_TX_COIL_INFO(chr2(1921:3000,:), 1, count*5);
  t.txCoilInfoAdult = read_TX_COIL_INFO(chr2(3001:4080,:), 1, count*5);
  t.numCoils = read_int(chr2(4081:4084,:), 1, count);
...
function [t, outpos] = read_GRADIENT_CONFIG_PARAM_TYPE(chr, inpos, count)
  ...
  t.coilAC_power_derated = read_int(chr2(17:20,:), 1, count);
...
function [t, outpos] = read_OP_PSC_START_TYPE(chr, inpos, count)
  outpos = inpos + 2104 * count;
  chr2=reshape(chr(inpos:inpos+count*2104-1), 2104, []);
  ...
  t.gradientConfigParam = read_GRADIENT_CONFIG_PARAM_TYPE(chr2(2085:2104,:), 1, count);
...
function [t, outpos] = read_OP_APS_START_TYPE(chr, inpos, count)
  outpos = inpos + 2144 * count;
  chr2=reshape(chr(inpos:inpos+count*2144-1), 2144, []);
  t.req_opcode = read_s32(chr2(1:4,:), 1, count);
  t.pass_desc = read_s32(chr2(5:8,:), 1, count);
  t.req_size = read_s32(chr2(9:12,:), 1, count);
  ...
  t.mr_min_signal = read_s32(chr2(113:116,:), 1, count);
  t.aps_peak_mode = read_s32(chr2(117:120,:), 1, count);
  t.bamtarget = read_s32(chr2(121:124,:), 1, count);
  t.target = read_s32(chr2(125:128,:), 1, count);
  t.min_signal = read_s32(chr2(129:132,:), 1, count);
  t.max_signal = read_s32(chr2(133:136,:), 1, count);
  t.breath_wait = read_s32(chr2(137:140,:), 1, count);
  t.discard_points = read_s32(chr2(141:144,:), 1, count);
  t.debug = read_s32(chr2(145:148,:), 1, count);
  t.min_hi_points = read_s32(chr2(149:152,:), 1, count);
  t.min_lo_points = read_s32(chr2(153:156,:), 1, count);
  t.min_noise = read_s32(chr2(157:160,:), 1, count);
  t.recv_strt = read_s32(chr2(161:164,:), 1, count);
  t.recv_end = read_s32(chr2(165:168,:), 1, count);
  t.timestamp = read_GEtimespec(chr2(169:176,:), 1, count);
  t.control = read_s32(chr2(177:236,:), 1, count*15);
  t.opnecho = read_s32(chr2(237:240,:), 1, count);
  t.filter1 = read_s32(chr2(241:244,:), 1, count);
  t.filter2 = read_s32(chr2(245:248,:), 1, count);
  t.noadc = read_s32(chr2(249:252,:), 1, count);
  t.nolr = read_s32(chr2(253:256,:), 1, count);
  t.acq_tagwords = read_s32(chr2(257:260,:), 1, count);
  t.cf_min_peak_threshold = read_s32(chr2(261:264,:), 1, count);
  t.suppress_pass = read_s32(chr2(265:268,:), 1, count);
  t.shim_pass = read_s32(chr2(269:272,:), 1, count);
  t.shim_step = read_s32(chr2(273:276,:), 1, count);
  t.spec_pts = read_s32(chr2(277:280,:), 1, count);
  t.spec_width = read_s32(chr2(281:284,:), 1, count);
  t.as_pass = read_s32(chr2(285:288,:), 1, count);
  t.plane_calc3 = read_s32(chr2(289:292,:), 1, count);
  t.def_plane = read_s32(chr2(293:296,:), 1, count);
  t.time_stamp = read_GEtimespec(chr2(297:304,:), 1, count);
  t.asxres = read_s32(chr2(305:308,:), 1, count);
  t.asyres = read_s32(chr2(309:312,:), 1, count);
  t.asbaseline = read_s32(chr2(313:316,:), 1, count);
  t.asrhblank = read_s32(chr2(317:320,:), 1, count);
  t.asptsize = read_s32(chr2(321:324,:), 1, count);
  t.opfov = read_f32(chr2(325:328,:), 1, count);
  t.transpose = read_s32(chr2(329:340,:), 1, count*3);
  t.rotation = read_s32(chr2(341:352,:), 1, count*3);
  t.cur_pos_x = read_s32(chr2(353:356,:), 1, count);
  t.cur_pos_y = read_s32(chr2(357:360,:), 1, count);
  t.cur_pos_z = read_s32(chr2(361:364,:), 1, count);
  t.cur_ax_x = read_s32(chr2(365:368,:), 1, count);
  t.cur_ax_y = read_s32(chr2(369:372,:), 1, count);
  t.cur_ax_z = read_s32(chr2(373:376,:), 1, count);
  t.cur_angle = read_s32(chr2(377:380,:), 1, count);
  t.field_strength = read_s32(chr2(381:384,:), 1, count);
  t.position = read_s32(chr2(385:388,:), 1, count);
  t.ftg_rg = read_s32(chr2(389:392,:), 1, count);
  t.fastrcv = read_s32(chr2(393:396,:), 1, count);
  t.vox_shim = read_s32(chr2(397:400,:), 1, count);
  t.boresize = read_s32(chr2(401:404,:), 1, count);
  t.ftg_start_ta = read_s32(chr2(405:408,:), 1, count);
  t.delta_TG = read_s32(chr2(409:412,:), 1, count);
  t.fseps_iter = read_s32(chr2(413:416,:), 1, count);
  t.fseps_mode = read_s32(chr2(417:420,:), 1, count);
  t.fseps_b0only_last_iter = read_s32(chr2(421:424,:), 1, count);
  t.fseps_slices = read_s32(chr2(425:428,:), 1, count);
  t.fseps_xres = read_s32(chr2(429:432,:), 1, count);
  t.fseps2_iter = read_s32(chr2(433:436,:), 1, count);
  t.isZ2 = read_s32(chr2(437:440,:), 1, count);
  t.isZ2autoshim = read_s32(chr2(441:444,:), 1, count);
  t.apa_coilsel = read_s32(chr2(445:448,:), 1, count);
  t.num_psc_vol = read_s32(chr2(449:452,:), 1, count);
  t.num_psc_slab = read_s32(chr2(453:456,:), 1, count);
  t.num_psc_rcvn_slab = read_s32(chr2(457:460,:), 1, count);
  t.cur_pos_vol2x = read_s32(chr2(461:464,:), 1, count);
  t.cur_pos_vol2y = read_s32(chr2(465:468,:), 1, count);
  t.cur_pos_vol2z = read_s32(chr2(469:472,:), 1, count);
  t.cur_ax_vol2x = read_s32(chr2(473:476,:), 1, count);
  t.cur_ax_vol2y = read_s32(chr2(477:480,:), 1, count);
  t.cur_ax_vol2z = read_s32(chr2(481:484,:), 1, count);
  t.ir_flag = read_s32(chr2(485:488,:), 1, count);
  t.escfh_slices = read_s32(chr2(489:492,:), 1, count);
  t.coil_weight = read_f32(chr2(493:1004,:), 1, count*128);
  t.fseps_rcvr = read_s32(chr2(1005:1008,:), 1, count);
  t.runReceiverNoise = read_s32(chr2(1009:1012,:), 1, count);
  t.max_rg = read_s32(chr2(1013:1016,:), 1, count);
  t.psc_prep = read_s32(chr2(1017:1020,:), 1, count);
  t.cfh_raise_threshold = read_s32(chr2(1021:1024,:), 1, count);
  t.indicate_single_peak = read_s32(chr2(1025:1028,:), 1, count);
  t.as_std_above_mean = read_f32(chr2(1029:1032,:), 1, count);
  t.as_max_2_floor = read_f32(chr2(1033:1036,:), 1, count);
  t.as_hard_floor = read_f32(chr2(1037:1040,:), 1, count);
  t.as_mask_ratio = read_f32(chr2(1041:1044,:), 1, count);
  t.apa_fov = read_f32(chr2(1045:1048,:), 1, count);
  t.fseps2_zffactor = read_f32(chr2(1049:1052,:), 1, count);
  t.receiverNoiseStdDev = read_f32(chr2(1053:1180,:), 1, count*32);
  t.cttEntry = read_ChannelTransTableEntryType(chr2(1181:2124,:), 1, count*4);
  t.apa_coilname = read_char(chr2(2125:2144,:), 1, count*20);

function [t, outpos] = read_OP_APS_START_PACK_TYPE(chr, inpos, count)
  outpos = inpos + 2184 * count;
  chr2=reshape(chr(inpos:inpos+count*2184-1), 2184, []);
  t.hdr = read_OP_HDR_TYPE(chr2(1:40,:), 1, count);
  t.req = read_OP_APS_START_TYPE(chr2(41:2184,:), 1, count);
...
function [t, outpos] = read_MRIMAGEDATATYPE(chr, inpos, count)
  ...
  t.temp_phases = read_int(chr2(1245:1248,:), 1, count);
  t.MEG_freq = read_int(chr2(1249:1252,:), 1, count);
  t.driver_amp = read_int(chr2(1253:1256,:), 1, count);
  t.driverCyc_Trig = read_int(chr2(1257:1260,:), 1, count);
  t.MEG_dir = read_int(chr2(1261:1264,:), 1, count);
  t.int_padding2 = read_int(chr2(1265:1368,:), 1, count*26);
  ...
  t.label_duration = read_unsigned_short_int(chr2(1553:1554,:), 1, count);
  t.short_padding = read_short_int(chr2(1555:1632,:), 1, count*39);
...
function [t, outpos] = read_RDB_HEADER_REC(chr, inpos, count)
  ...
  t.rdb_hdr_numbvals = read_short(chr2(2663:2664,:), 1, count);
  t.rdb_hdr_numdifdirs = read_short(chr2(2665:2666,:), 1, count);
  t.rdb_hdr_difnext2 = read_short(chr2(2667:2668,:), 1, count);
  t.rdb_hdr_difnextab = read_short(chr2(2669:2868,:), 1, count*100);
  t.rdb_hdr_channel_combine_method = read_short(chr2(2869:2870,:), 1, count);
  t.rdb_hdr_nexForUnacquiredEncodes = read_short(chr2(2871:2872,:), 1, count);
  t.rdb_hdr_excess = read_short(chr2(2873:4096,:), 1, count*612);
