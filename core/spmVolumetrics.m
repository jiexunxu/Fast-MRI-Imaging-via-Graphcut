function [spmLabelMap, x, segMap]=spmVolumetrics(x)
...
function [spmLabelMap, segMap]=RunAtlasing_SVN(T1FileName, atlassize)
    ...
    [AtlasedFileName, segMap] = Atlasing_spm8_SVN(T1FileName, StrOutputDir, AtlasFileName, Thresh,SPMdir,atlassize,1);
...
function [AtlasedFile, segMap] = Atlasing_spm8_SVN(Images, Output_dir, AtlasFile, Thresh, SPMdir, atlassize,affinereg)
    spm_get_defaults;
    warning off;
    global defaults;
    ...
    segMap=zeros(size(Vcsf.private.dat));
    segMap(double(Vcsf.private.dat)>0)=atlassize+1;
    segMap(double(Vwhite.private.dat)>0)=atlassize+2;
end
