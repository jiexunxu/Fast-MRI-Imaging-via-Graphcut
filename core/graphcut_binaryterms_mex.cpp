 ...
 * [unaryTerms, binaryTerms, binaryTermsCounter]=graphcut_binaryterms_mex(size(unaryTerms, 1), unaryTerms(:), binaryTermsCounter, ...
    size(binaryTerms, 1), binaryTerms(:), size(nbrList, 1), nbrList(:)-1, ...
    real(x0), imag(x0), size(V, 1), vxlSpts, real(full(vxlWeights)), imag(full(vxlWeights)), objParams(4), objParams(5), objParams(2));
 */
double EsWeight;
double* vxlSpts;
double* vxlWeightsReal;
double* vxlWeightsImag;
mwSize numSegs;
bool debugPrint=false;

inline int s1s2ToBinaryIdx(int s1, int s2)
{
    return (s1-1)*numSegs+s2-1;    
}
 ...
 //   extractMaxSupport(supportPtrs, vxlSptCounts, v2, s2, w2real, w2imag);
    ...
    }else if((s1!=-1)&&(s2!=-1))
    {
        if(s1==s2)
        {
            ...
            int bIdx=binaryTermsIdx-1+s1s2ToBinaryIdx(s1, s2);
            binaryTermsOut[bIdx]=s1;
            binaryTermsOut[bIdx+binaryTermsCount]=s2; 
            binaryTermsOut[bIdx+binaryTermsCount*2]+=termWeight*singlePairEs(power, Es_threshold, x1real, x1imag, x2real, x2imag);
            binaryTermsOut[bIdx+binaryTermsCount*3]+=termWeight*singlePairEs(power, Es_threshold, x1real+w1real, x1imag+w1imag, x2real, x2imag);
            binaryTermsOut[bIdx+binaryTermsCount*4]+=termWeight*singlePairEs(power, Es_threshold, x1real, x1imag, x2real+w2real, x2imag+w2imag);
            binaryTermsOut[bIdx+binaryTermsCount*5]+=termWeight*singlePairEs(power, Es_threshold, x1real+w1real, x1imag+w1imag, x2real+w2real, x2imag+w2imag);  
            /*
