...
% mu = 0.001;      
function x0=estimateInitialSolution(data, sensit, G, sigma, lambda, maxIter)
...
%{
    xInitWavelet=zeros(ylength, 1);
    xtrunkCounter=1;
    for j=1:length(W.dec)
        xInitWavelet(xtrunkCounter:xtrunkCounter+numel(W.dec{j})-1)=W.dec{j}(:);
        xtrunkCounter=xtrunkCounter+numel(W.dec{j});
    end   
%}
  ...
  %  x0Wavelet=spg_bpdn(@HxTV, b, norm(b)*1e-1, opts);
    ...
    function y=HxTV(x, mode)
        if mode==1
            ...
            xtmp=xtmp(:)+fullNbrMat'*x(M+1:length(x));
