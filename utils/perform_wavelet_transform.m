...
[m,J] = dyadlength(wc(:,1));
[n,K] = dyadlength(wc(1,:));
% assume m==n, J==K (freaking STUPID assumption)
...
for jscal=L:J-1,
    botrow = 1:dpm(jscal+1);
    toprow = (dpm(jscal+1)+1):dpm(jscal+2);
    botcol = 1:dpn(jscal+1);
    topcol = (dpn(jscal+1)+1):dpn(jscal+2);
    
    allrows = [botrow toprow];
    allcols = [botcol topcol];
    
    nc = length(allcols);
    for iy=1:nc
        x(allrows,iy) =  UpDyad_SBS(x(botrow,iy)', x(toprow,iy)', qmf, dqmf)';
    end
    nc = length(allrows);
    for ix=1:nc
        x(ix,allcols) = UpDyad_SBS(x(ix,botcol), x(ix,topcol), qmf, dqmf);
    end
end
